...
- Allowed media keys are generated by SuqPage for same-request attachments and
  authorized same-tenant current media. Imports cannot use URLs, filesystem
  paths, media IDs, another request's key, or another tenant's asset.
