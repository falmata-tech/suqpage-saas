...
- A host qualifies when its local group contains at least three matching
  ...
  three, the largest nearest-host group becomes the sole City Suq so results do
  not disappear.
...
Scenario: No city reaches the minimum
  ...
  THEN both businesses appear in one deterministic City Suq
...
Scenario: Ineligible business is requested
  ...
  THEN it contributes to no count, City Suq, booth, or list row
```
...
- `scripts/test-scale-fixtures.ts` passed 58 seeded Suqs, six industries, 54
  currently entitled discovery participants, four payment-lifecycle exclusions,
  exactly ten featured profiles, and bounded City Suq halls.
