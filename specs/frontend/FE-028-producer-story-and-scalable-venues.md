...
status: in_progress
related: [FE-021, FE-024, FE-025, FE-027, FE-029, FE-030]
...
The public story is earnest but generic, and the virtual City Showroom and Daily Featured
...
- Distinct Daily Featured and City Showroom art direction with landscaped entry plazas,
...
- A cohesive architectural hall shell with real walls, glass entrance,
  ...
  open floor.
- Automatic numbered booth/shop rows with regular walking gaps, wider
  cross-aisles on larger floors, and architectural wall clearance.
...
- Changing eligibility, clustering, Daily Featured scheduling, redaction, inquiry, or
...
- Daily Featured and City Showroom are virtual discovery spaces, not physical locations.
...
- Daily Featured and City floors use different surface accents while sharing the same
...
- Every booth/shop position is computed in deterministic left-to-right,
  top-to-bottom exhibition rows. Adding businesses grows the floor by rows,
  never changes the architectural image, and never assigns two businesses to
  one position.
- Daily Featured cards are structured as physical kiosk fronts with a fascia, framed
  approved image, information panel, and shallow floor-mounted platform. The
  platform is part of the interactive booth, not decorative venue artwork.
- Booth references are visibly prominent and follow the same row-major order as
  the computed exhibition positions.
...
Scenario: Visitor understands the MirtPage point of view
  ...
  THEN MirtPage connects Ethiopian production to households and trade buyers
  AND direct purchase, wholesale sourcing, and distribution value are clear without generic slogans

Scenario: Daily Featured grows without losing its setting
  GIVEN any positive number of eligible Daily Featured booths
...
| Daily Featured and City desktop/mobile appearance and interaction | browser/manual | `scripts/capture-discovery-visuals.mjs` |
...
- `npm run test:acceptance` passed all 10 ordered production-browser workflows,
  including responsive discovery, continuous Daily Featured behavior, City Showroom
...
- Prior, superseded layout evidence: the architectural-shell implementation passed `npm run check` and a
...
- Those prior captures show the complete enclosed hall at desktop, 390px,
  ...
  and empty center. FE-030 replaces that perimeter geometry with deterministic
  exhibition rows and no empty-center requirement.
- Reviewed captures in `/tmp/mirtpage-discovery-visuals/` confirm the Daily Featured and
