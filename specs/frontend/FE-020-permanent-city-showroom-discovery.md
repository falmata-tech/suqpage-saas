---
id: FE-020
title: Permanent industry and City Showroom discovery
status: deprecated
...
# FE-020 - Permanent industry and City Showroom discovery

Deprecated by `FE-021`, `BE-020`, and `DEP-017`. This file retains the verified
historical City Showroom contract; it is not current public behavior.
...
industry, searches the same result set, explores matching City Showrooms on the
Ethiopia map, and enters each business's permanent Showroom.
...
- A persistent local Ethiopia map with matching City Showroom markers and counts.
- Direct city buttons and a country reset; selecting a city keeps the map
  mounted and reveals its virtual City Showroom over restrained geographic context.
- A simple top-view City Showroom with a circular central lobby/planter, clear ring
  or radial corridors, numbered booths, and at most twelve visible booths per
  hall.
- Hall controls for larger City Showrooms, complete list parity, and permanent
  `/@handle` links labeled **Visit Showroom**.
...
- An application-first homepage where a compact merchant invitation leads
  ...
  marketplace.
- A compact featured-Showroom rail and touch-friendly City Showroom buttons derived from
  the same active industry/search projection as Map View and List View.
- A bounded City Showroom overlay that leaves recognizable Ethiopia map context
...
- **Showroom:** one business's permanent `/@handle` public page.
- **City Showroom:** a virtual discovery venue for one selected industry and serving
...
- Public copy may explain a Showroom as a professional digital showroom, but actions
  say **Find a Showroom**, **Visit Showroom**, and **Get your MirtPage**.
...
- The country map renders hub summaries, not business points. Booths appear
  only inside the selected City Showroom.
...
Scenario: Visitor changes industry
  ...
  THEN the selected state, City Showroom counts, featured emphasis, map, and list all
...
Scenario: Visitor uses marketplace shortcuts
  ...
  WHEN the visitor uses a featured card or City Showroom button
  THEN the shortcut represents the same industry and search projection
  AND it opens the permanent Showroom or selected City Showroom without duplicate filters
...
Scenario: Visitor opens a City Showroom on a phone
...
Scenario: Map assets fail
  ...
  AND every matching permanent Showroom remains reachable
```
...
| Nearest City Showroom, hall paging, stable references | integration | `scripts/test-discovery.ts` |
...
- Desktop and phone City Showrooms rendered five balanced booths, four planters, two
...
- `tests/acceptance/app.spec.ts` passed 10/10 production-browser scenarios,
  including first-viewport discovery, featured and city shortcuts, City Showroom
