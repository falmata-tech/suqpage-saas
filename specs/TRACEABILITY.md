...
| Reproducible delivery and repository hygiene | — | — | DEP-002 | `.dockerignore`, `Dockerfile`, `compose.yaml`, `.github/workflows/quality.yml`, `next.config.ts`, `lib/config.ts`, `lib/media.ts`, `package.json`, `scripts/release.sh` | `scripts/test-container.mjs`, `scripts/test-workflow.mjs`, `scripts/test-build-trace.mjs`, `scripts/test-security.ts`, `scripts/test-operations.mjs`, `tests/acceptance/app.spec.ts`, `npm run release`, GitHub Actions `29889083549` | done |
| Managed client requests, staff fulfillment, preview approval, and publication | FE-003 | BE-003 | DEP-003 | Active increment: attachment-free public interest, invitation/client workspace, private images, staff provisioning, on-behalf intake, assignment, structured immutable revisions, exact client decisions, atomic approved publication, retained rollback, schema migrations 2–6. Remaining: clarification messaging and legacy-owner cutover | `scripts/test-requests.ts`, `scripts/test-revisions.ts`, `scripts/test-security.ts`, `scripts/http-smoke.mjs`, `scripts/test-operations.mjs`, `tests/acceptance/app.spec.ts`; cutover gates remain | in_progress |
