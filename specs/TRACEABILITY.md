...
| Managed client requests, staff fulfillment, preview approval, and publication | FE-003 | BE-003 | DEP-003 | Active increment: attachment-free public interest, single-use invitation, restricted client workspace/request history, private images, individual staff provisioning, manager on-behalf intake, least-privilege assignment, schema migrations 2–5. Remaining: versioned revision approval/publication and legacy-owner cutover | `scripts/test-requests.ts`, `scripts/test-security.ts`, `scripts/http-smoke.mjs`, `scripts/test-operations.mjs`, `tests/acceptance/app.spec.ts`; revision/cutover gates remain | in_progress |
