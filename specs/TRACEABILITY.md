...
| Managed client requests, staff fulfillment, preview approval, and publication | FE-003 | BE-003 | DEP-003 | Active increment: attachment-free public interest, operations review, capability profiles, single-use invitation, restricted client workspace/request history, authenticated private images, schema migrations 2–4. Remaining: staff/manager provisioning and assignment, on-behalf intake, revision approval/publication | `scripts/test-requests.ts`, `scripts/test-security.ts`, `scripts/http-smoke.mjs`, `scripts/test-operations.mjs`, `tests/acceptance/app.spec.ts`; revision/cutover gates remain | in_progress |
