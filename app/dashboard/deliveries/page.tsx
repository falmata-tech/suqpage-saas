...
import { getInquiry, listDeliveryCompanies, listDeliveryRequests, listInquiries } from "@/lib/db";

export const dynamic = "force-dynamic";

export default async function DeliveriesPage({ searchParams }: { searchParams:Promise<{business?:string;inquiry?:string;created?:string;error?:string}> }) {
  ...
  const inquiries = client ? [] : listInquiries(business.id);
  ...
  return <DashboardShell user={user} business={business}>
    ...
    <section className="panel"><h2>Request history</h2>{deliveries.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Request</th><th>Customer</th><th>Route</th><th>Packages</th><th>Status</th><th>Created</th></tr></thead><tbody>{deliveries.map((delivery:any) => <tr key={delivery.id}><td><strong>{delivery.external_request_id}</strong></td><td>{delivery.customer_name}<br/><small>{delivery.phone}</small></td><td>{delivery.pickup_address}<br/>→ {delivery.delivery_address}</td><td>{delivery.package_count}</td><td><span className={`badge ${delivery.status}`}>{delivery.status}</span></td><td>{new Date(delivery.created_at).toLocaleString()}</td></tr>)}</tbody></table></div> : <div className="empty-state">No delivery requests yet.</div>}</section>
  </DashboardShell>;
}
