...
export default async function OnBehalfRequestPage({
  ...
  return <DashboardShell user={user} business={business}>
    <NavigationTrail items={[{label:business ? "Page project" : "Page requests",href:requestsHref},{label:business ? requestType==="onboarding"?"Create page":"Update page" : "Record request"}]} fallback={requestsHref}/>
    <div className="dashboard-head"><div><h1>{business ? `${requestType==="onboarding"?"Create":"Update"} page for ${business.name}` : "Record a page request"}</h1><p>{business ? "Record the intended outcome and begin one private page project in this business workspace." : "Find an existing business or continue with no selection to capture a new prospect. AfricMade is shown as the submitter."}</p></div></div>
