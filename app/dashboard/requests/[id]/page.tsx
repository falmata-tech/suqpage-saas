...
export default async function RequestDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; created?: string; assigned?:string; clarified?:string; published?:string; rolledBack?:string; error?: string; staffQ?: string }> }) {
  ...
  const projectTitle=request.request_type==="onboarding"?"Page setup":"Page update";
  ...
  return <DashboardShell user={user} business={business}>
    <NavigationTrail items={[{label:business?"Page project":manager?"Operations":"Assigned requests",href:projectsHref},{label:projectTitle}]} fallback={projectsHref}/>
    ...
    {query.created && <p className="notice">{query.created === "manager" ? "The page project was started for the client." : "Your page project was sent to AfricMade."}</p>}{query.saved && <p className="notice">Project status updated.</p>}{query.assigned && <p className="notice">Assignment updated.</p>}{query.clarified && <p className="notice">Clarification message added.</p>}{"published" in query && <p className="notice">The approved revision is now live.</p>}{"rolledBack" in query && <p className="notice">The previous page version was restored and published.</p>}{query.error && <p className="error">{query.error==="clarification"?"The clarification message could not be added.":query.error}</p>}
    <div className="split"><section className="panel"><h2>{request.request_type === "change" ? "Requested changes" : "Setup brief"}</h2><dl className="request-facts"><dt>Contact name</dt><dd>{request.contact_name}</dd><dt>Contact</dt><dd>{request.contact_value}</dd><dt>Business</dt><dd>{request.business_display_name || request.business_name || "Not provided"}</dd><dt>Started by</dt><dd>{request.submitter_kind === "manager" ? "AfricMade team" : request.submitter_kind}</dd>{request.assigned_user_name && !client ? <><dt>Assigned to</dt><dd>{request.assigned_user_name}</dd></> : null}</dl><h3>{request.request_type === "change" ? "Requested outcome" : "Original instruction"}</h3><p className="request-copy">{request.request_text}</p></section>{manager || teamMember ? <section className="panel"><h2>Work status</h2><form action={updateServiceRequestStatusAction} className="form-grid"><input type="hidden" name="requestId" value={request.id}/><div className="field full"><label htmlFor="request-status">Status</label><select id="request-status" name="status" defaultValue={request.status}>{REVIEW_REQUEST_STATUSES.map((status) => <option value={status} key={status}>{status.replaceAll("_"," ")}</option>)}</select></div><div className="field full"><button className="btn">Update status</button></div></form><p><small>Prepare changes in a private revision. The published page stays unchanged until approval.</small></p></section> : <section className="panel"><h2>What happens next</h2><p>AfricMade reviews your instruction, asks for clarification if needed, and prepares private work for your approval. Nothing becomes public without your approval.</p></section>}</div>
    ...
    {request.business_id ? (
      <section className="panel">
        <div className="dashboard-head">
          <div><h2>Private revisions</h2><p>Numbered previews stay separate from the live page until the client approves them and a manager publishes them.</p></div>
          {canAuthor && draftRevision ? <div className="inline-actions"><Link className="btn brand" href={`/dashboard/requests/${request.id}/revisions/${draftRevision.id}/edit`}>Edit current page</Link><Link className="btn secondary" href={`/dashboard/requests/${request.id}/revisions/${draftRevision.id}/studio`}>AI-assisted redesign</Link></div> : null}
        </div>
        {canAuthor && !draftRevision ? request.request_type === "change" ? (
          <div className="showroom-authoring-choices" aria-label="Choose an update tool">
            <form action={createRevisionDraftAction} className="showroom-authoring-choice"><input type="hidden" name="requestId" value={request.id}/><input type="hidden" name="workspace" value="edit"/><div><strong>Edit current page</strong><span>Keep the current design and change only the copy, images, offerings, components, colors, or motion that need attention.</span></div><button className="btn brand">Start editing</button></form>
            <form action={createRevisionDraftAction} className="showroom-authoring-choice"><input type="hidden" name="requestId" value={request.id}/><input type="hidden" name="workspace" value="studio"/><div><strong>AI-assisted redesign</strong><span>Use a complete AI recipe when the page needs a new structure, visual direction, or broad content rewrite.</span></div><button className="btn secondary">Open AI design</button></form>
          </div>
        ) : (
          <form action={createRevisionDraftAction} className="showroom-setup-action"><input type="hidden" name="requestId" value={request.id}/><input type="hidden" name="workspace" value="studio"/><button className="btn brand">Prepare first page design</button></form>
        ) : null}
        {revisions.length ? (
              ...
              <div className="staff-card" key={revision.id}>
                ...
                {revision.status === "draft" && (manager || teamMember) ? null : (
                  <div className="inline-actions">
                    {revision.status === "draft" ? <span className="muted">AfricMade is preparing this revision.</span> : <Link className="small-btn" href={`/dashboard/requests/${request.id}/revisions/${revision.id}/preview`}>View preview</Link>}
    ...
    {request.submitter_kind !== "public" && request.attachments.length ? <section className="panel"><h2>Page images</h2><p>Images uploaded through this design&apos;s labeled checklist.</p><div className="request-image-grid">{request.attachments.map((attachment) => <figure key={attachment.id}><img src={`/api/requests/${request.id}/attachments/${attachment.id}`} alt={attachment.original_name}/><figcaption>{attachment.original_name} · {attachment.width}×{attachment.height}</figcaption></figure>)}</div></section> : null}
    <section className="panel"><h2>Clarifications</h2>{clarifications.length?<ol className="request-events bounded-record-list" aria-label="Clarification messages" tabIndex={0}>{clarifications.map((event)=>{const own=event.actor_user_id===user.id;const label=user.access_role==="client"?(event.actor_access_role==="client"?(own?"You":"Client"):"AfricMade team"):(event.actor_name||"AfricMade team");return <li key={event.id}><strong>{label}</strong><span>{event.detail}</span><time>{new Date(event.created_at).toLocaleString()}</time></li>})}</ol>:<div className="empty-state">No clarification messages yet.</div>}{clarificationOpen?<form action={addRequestClarificationAction} className="form-grid"><input type="hidden" name="requestId" value={request.id}/><div className="field full"><label htmlFor="clarification-message">{user.access_role==="client"?"Reply to AfricMade":"Ask or answer a clarification"}</label><textarea id="clarification-message" name="message" required maxLength={2000} placeholder="Add information without changing the original request."/></div><div className="field full"><button className="btn">Add clarification</button></div></form>:null}</section>
