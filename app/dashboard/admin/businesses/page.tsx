...
export default async function AdminBusinesses({ searchParams }: {
  ...
  return <DashboardShell user={user} business={null}>
    ...
    <div className="dashboard-head"><div><span className="eyebrow">Customer operations</span><h1>Accounts</h1><p>Find any account, then manage its AfricMade page, marketplace presence, access, activity, and service record in one workspace.</p></div><Link className="btn brand" href="/dashboard/clients/new">Add account</Link></div>
    ...
    {businesses.items.length ? <>
      <div className="table-wrap admin-data-surface"><table className="data-table"><thead><tr><th>Account</th><th>Access</th><th>Marketplace</th><th>Requests</th><th>Page</th><th><span className="sr-only">Workspace</span></th></tr></thead><tbody>{businesses.items.map((business)=><tr key={business.id}>
        ...
        <td data-label="Page"><span className={`badge ${business.status}`}>{business.status}</span></td>
