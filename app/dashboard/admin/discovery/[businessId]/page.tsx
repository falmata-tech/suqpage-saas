...
export default async function DiscoveryProfilePage({ params, searchParams }: {
  ...
  return <DashboardShell user={user} business={null}>
    ...
    <div className="dashboard-head"><div><span className="eyebrow">Business discovery profile</span><h1>{profile.businessName}</h1><p>@{profile.handle} · {profile.city || "Origin incomplete"}</p></div><Link className="btn secondary" href={`/preview/@${profile.handle}`} target="_blank">Open showroom</Link></div>
    {query.error ? <p className="error">{query.error}</p> : null}
    {query.saved ? <p className="notice">Discovery profile saved.</p> : null}
    <div className="split focused-expo-layout">
      <section className="panel"><h2>Visibility summary</h2><p><span className={`badge ${profile.excluded ? "cancelled" : profile.approved ? "active" : "limited"}`}>{profile.excluded ? "excluded" : profile.approved ? "discoverable" : "incomplete"}</span></p><dl className="request-facts"><dt>Industries</dt><dd>{profile.industryKeys.join(", ") || "Not assigned"}</dd><dt>Origin</dt><dd>{[profile.city, profile.zone, profile.region].filter(Boolean).join(", ") || "Not set"}</dd><dt>Sponsored</dt><dd>{profile.sponsored ? `Yes · position ${profile.sponsorPosition}` : "No"}</dd><dt>Sunday program</dt><dd>{profile.sundayIndustryKeys.join(", ") || "Not selected"}</dd></dl></section>
      <section className="panel"><h2>Edit discovery profile</h2><form action={updateDiscoveryProfileAction} className="form-grid"><input type="hidden" name="businessId" value={profile.businessId}/><fieldset className="field full"><legend>Industries</legend><div className="recipe-checks">{DISCOVERY_INDUSTRIES.map((industry) => <label key={industry.key} className="check-field"><input type="checkbox" name="industryKeys" value={industry.key} defaultChecked={profile.industryKeys.includes(industry.key)}/>{industry.label}</label>)}</div></fieldset><div className="field"><label htmlFor="profile-scale">Production scale</label><select id="profile-scale" name="productionScale" defaultValue={profile.productionScale}><option value="workshop">Workshop / producer</option><option value="growing_factory">Growing factory</option></select></div><div className="field full"><label htmlFor="profile-image">Approved booth image path</label><input id="profile-image" name="boothImagePath" defaultValue={profile.boothImagePath} placeholder="/uploads/..." required/></div><div className="field"><label htmlFor="profile-city">City</label><input id="profile-city" name="city" defaultValue={profile.city} required/></div><div className="field"><label htmlFor="profile-zone">Zone / administrative area</label><input id="profile-zone" name="zone" defaultValue={profile.zone} required/></div><div className="field full"><label htmlFor="profile-region">Region</label><input id="profile-region" name="region" defaultValue={profile.region} required/></div><div className="field"><label htmlFor="profile-latitude">Latitude</label><input id="profile-latitude" name="latitude" type="number" min="3" max="15" step="any" defaultValue={profile.latitude ?? ""} required/></div><div className="field"><label htmlFor="profile-longitude">Longitude</label><input id="profile-longitude" name="longitude" type="number" min="32" max="49" step="any" defaultValue={profile.longitude ?? ""} required/></div><div className="field full"><label htmlFor="profile-fallback">Fallback style</label><select id="profile-fallback" name="fallbackStyle" defaultValue={profile.fallbackStyle}>{["workshop", "botanical", "textile", "food", "home", "technical"].map((style) => <option key={style}>{style}</option>)}</select></div><fieldset className="field full"><legend>Marketplace programs</legend><div className="recipe-checks"><label className="check-field"><input type="checkbox" name="sponsored" defaultChecked={profile.sponsored}/>Sponsored placement (paid)</label></div><div className="form-grid"><div className="field"><label htmlFor="sponsor-position">Sponsored position</label><input id="sponsor-position" name="sponsorPosition" type="number" min="1" max="999" defaultValue={profile.sponsorPosition} required/></div><div className="field"><label htmlFor="sunday-position">Sunday position</label><input id="sunday-position" name="sundayPosition" type="number" min="1" max="999" defaultValue={profile.sundayPosition} required/></div></div><p className="muted">Featured Enterprises is selected by MirtPage independently from sponsorship.</p><div className="recipe-checks">{DISCOVERY_INDUSTRIES.map((industry) => <label key={industry.key} className="check-field"><input type="checkbox" name="sundayIndustryKeys" value={industry.key} defaultChecked={profile.sundayIndustryKeys.includes(industry.key)}/>{industry.label}</label>)}</div></fieldset><label className="check-field"><input type="checkbox" name="excluded" defaultChecked={profile.excluded}/>Exclude from discovery</label><div className="field full"><button className="btn brand">Save discovery profile</button></div></form></section>
    </div>
  </DashboardShell>;
}
