...
export default async function ShowroomPage({params,searchParams}:{params:Promise<{handle:string}>;searchParams:Promise<{ref?:string}>}){const handle=cleanHandle((await params).handle);const catalog=await runtimeCatalogByHandle(handle);if(!catalog){const existing=await runtimeBusinessByHandleAny(handle);if(existing?.status==="suspended")redirect("/?showroom=inactive");notFound();}const query=await searchParams;const source=query.ref==="featured"?"featured":query.ref==="discovery"?"directory":"direct";return <><ShowroomVisitBeacon handle={catalog.business.handle} source={source}/><ShowroomApp catalog={catalog} withinPublicApp/></>}
