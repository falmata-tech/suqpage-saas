...
import { postgresRuntimePreviewEnabled, postgresRuntimeServices } from "@/lib/postgres-runtime-services";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createPublicInterest(values, hashPrivateValue(requestIpFromHeaders(request.headers)), {
      repository: postgresRuntimePreviewEnabled() ? postgresRuntimeServices().requests : new SqliteRequestRepository(), rateLimiter: new PublicRequestRateLimiter(),
    });
