...
export default async function InvitationPage({ params, searchParams }: { params:Promise<{token:string}>; searchParams:Promise<{error?:string}> }) {
  ...
  const errors: Record<string,string> = {
    ...
    expired: "This invitation expired or was replaced. Ask AfricMade for a new link.",
    replayed: "This invitation has already been used.",
    conflict: "An account already exists for this email. Contact AfricMade for help.",
    invalid: "The invitation could not be accepted.",
  };
  return <><main className="invite-page public-mobile-page"><div className="invite-card">
    <NavigationTrail items={[{label:"AfricMade",href:"/"},{label:"Client invitation"}]} fallback="/"/>
    {!invitation ? <><span className="eyebrow">Invitation unavailable</span><h1>Ask AfricMade for a new invitation</h1><p>This link is invalid, expired, already used, or was replaced for your protection.</p><a className="btn secondary" href="/">Return home</a></> : <>
      <span className="eyebrow">Private client invitation</span><h1>Join {invitation.business_name}</h1><p>Set up your private client workspace. You’ll use it to send requests, follow inquiries, manage offerings, contact support, and review page previews.</p>
  ...
  </div></main><PublicMobileNavigation /></>;
}
