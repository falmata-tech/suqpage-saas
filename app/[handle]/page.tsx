...
export default async function ShowroomPage({params,searchParams}:{params:Promise<{handle:string}>;searchParams:Promise<{ref?:string}>}){const handle=cleanHandle((await params).handle);const existing=getBusinessByHandleAny(handle);if(existing?.status==="suspended")redirect("/?showroom=inactive");const catalog=getCatalogByHandle(handle);if(!catalog)notFound();const query=await searchParams;const source=query.ref==="expo"?"expo":query.ref==="discovery"?"directory":"direct";return <><ShowroomVisitBeacon handle={catalog.business.handle} source={source}/><ShowroomApp catalog={catalog}/></>}
