# Tana Cloth House showroom brief

> Fictional disposable client used to exercise the SuqPage design workflow. All copy and imagery are demonstration material, not verified business claims.

## Identity

- Business name: Tana Cloth House
- Permanent handle: `demo-tana-cloth-house`
- Location: Bahir Dar, Amhara
- Existing logo/mark: generated geometric mark based on a folded T made from a measured cloth roll
- Short description: Show the breadth without turning the page into a crowded fabric shop.

## Customer And Goal

- Primary customer: households, tailors, and small retailers
- Main visitor decision: identify the closest offering and decide what requirement to send
- Inquiry outcome: one direct SuqPage inquiry with optional quantity and practical context
- Product, production, or custom-work mode: mixed as recorded per offering
- Information that must appear early: what the business makes or does, who it serves, and the four available starting points

## Brand Direction

- Customer color request: teal, orange, and cream
- Designer advice: Teal and orange remain but sit on crisp white and cool gray, giving the products more separation than a cream base.
- Logo concept: a folded T made from a measured cloth roll
- Visual thesis: A clean shelf of cloth families with strong color-coded navigation.
- Visual directions to avoid: one-color tinting, generic stock scale, framed hero boxes, decorative clutter, unsupported claims

## Final Palette

- canvas: `#ffffff`
- surface: `#f3f6f8`
- layer: `#e4eaf0`
- text: `#17212b`
- textMuted: `#596572`
- inverse: `#111820`
- onInverse: `#ffffff`
- border: `#c7d0d8`
- primary: `#c9592c`
- primarySoft: `#f3ddd3`
- secondary: `#17616a`
- secondarySoft: `#d5e9eb`
- onSecondary: `#ffffff`
- strong: `#17616a`
- onStrong: `#ffffff`

## Content Inventory

- Products or capabilities: Everyday Garment Cotton (Garment Cloth); Durable Sheet Fabric (Household Cloth); Textured Curtain Cloth (Interior Cloth); Coordinated Home Cloth Pack (Household Sets)
- Required inquiry details: intended use, format or specification, timing, and optional quantity
- Unknowns: all operational facts remain provisional because this is a fictional client

## Media Authority

| File | Subject | Factual Or Illustrative | Rights Confirmed | Preferred Role |
|---|---|---|---|---|
| `/uploads/seed/portfolio/demo-tana-cloth-house/hero.webp` | business-specific working scene | illustrative demo artwork | yes | hero |
| `/uploads/seed/portfolio/demo-tana-cloth-house/product-1.webp` | Everyday Garment Cotton | illustrative demo artwork | yes | offering |
| `/uploads/seed/portfolio/demo-tana-cloth-house/product-2.webp` | Durable Sheet Fabric | illustrative demo artwork | yes | offering |
| `/uploads/seed/portfolio/demo-tana-cloth-house/product-3.webp` | Textured Curtain Cloth | illustrative demo artwork | yes | offering |
| `/uploads/seed/portfolio/demo-tana-cloth-house/product-4.webp` | Coordinated Home Cloth Pack | illustrative demo artwork | yes | offering |
| `/landing/showroom-booths/demo-tana-cloth-house.svg` | coordinated virtual venue booth | illustrative demo artwork | yes | discovery booth |

## Composition Direction

- Foundation choice: `ocean-trade`
- Foundation reason: ocean-trade supplies the typography, spacing, geometry, and media behavior because the brief fits utility content anatomy; the palette is independently authored from the brand direction.
- Header and hero needs: compact identity, immediate offer, and integrated wide media with protected copy space
- Story/process distinction: opposing alignment and contrasting semantic surfaces without divider clutter
- Catalog density and media behavior: exactly four comparable cards with bounded 4:3 media
- Inquiry CTA and floating control: persistent but clear of phone content
- Footer needs: identity and contact close without category navigation

## Booth Direction

- Approved logo/name treatment: generated mark plus exact business name
- Facade material and color: shared clean City Suq architecture with the final primary and secondary roles
- Product/craft cue: a folded T made from a measured cloth roll
- Generated image authorized: yes, for disposable demonstration use

## Acceptance Notes

- Desktop priorities: integrated hero, clean section rhythm, visible product comparison, decisive CTA
- Phone priorities: no horizontal overflow, readable logo, 44px actions, unobstructed inquiry control
- Accessibility/localization risks: long business names, custom palette contrast, reduced motion
- Client review questions: replace every provisional fact and illustration before any real-client publication
