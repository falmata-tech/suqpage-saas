...
export default function OnBehalfRequestForm({ client, business, requestType }: { client?: ManagedClientRow; business?: Business; requestType?: ServiceRequestType }) {
  ...
  return <form className="panel form-grid" onSubmit={submit}>
    ...
    <div className="field full"><span className="eyebrow">{isUpdate?"Page update":"Page setup"}</span><h2>{isUpdate ? "What should change?" : "Describe the first page"}</h2><p>{isUpdate ? "Record the requested outcome without repeating information already on the live page." : "AfricMade selects setup or update from the account's publication history."}</p></div>
    ...
    <div className="field"><label htmlFor="behalf-catalog-stage">Catalog stage</label><select id="behalf-catalog-stage" name="catalogStage" defaultValue="Let AfricMade choose"><option>Let AfricMade choose</option><option>Small catalog (1-5)</option><option>Growing catalog (6-15)</option><option>Large catalog (16+)</option></select></div>
    ...
    <div className="field full"><button className="btn brand" disabled={pending}>{pending ? "Starting project…" : existingBusiness ? isUpdate ? "Start page update" : "Start page setup" : "Record prospect request"}</button></div>
  </form>;
}
