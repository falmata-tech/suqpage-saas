...
export default function OnBehalfRequestForm({ clients }: { clients:ManagedClient[] }) {
  ...
  const [requestType,setRequestType] = useState("onboarding");
  ...
  return <form className="panel form-grid" onSubmit={submit} encType="multipart/form-data">
    ...
    <div className="field full"><label htmlFor="behalf-type">Request type</label><select id="behalf-type" name="requestType" value={requestType} onChange={(event)=>setRequestType(event.target.value)} disabled={!existingClient}><option value="onboarding">First showroom build</option><option value="change">Showroom change</option></select>{!existingClient ? <input type="hidden" name="requestType" value="onboarding"/> : null}</div>
