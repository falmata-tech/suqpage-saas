...
export default function OnBehalfRequestForm({ client }: { client?: ManagedClientRow }) {
  ...
  return <form className="panel form-grid" onSubmit={submit}>
    ...
    <div className="field full"><span className="eyebrow">{client?.request_type==="change"?"Showroom change request":"New showroom request"}</span><p>MirtPage selects the correct request type from the business&apos;s current showroom status.</p></div>
    ...
    <div className="field full"><label htmlFor="behalf-photography-stage">Photography</label><select id="behalf-photography-stage" name="photographyStage" defaultValue="Some images exist; create labeled slots for the rest"><option>Images are ready for the design checklist</option><option>Some images exist; create labeled slots for the rest</option><option>Photography will be added during design</option></select></div>
