...
function ShowroomImage({ showroom }: { showroom: DiscoveryShowroom }) {
  ...
  return showroom.heroImagePath && !failed
    ? <Image src={showroom.heroImagePath} alt="" width={480} height={300} sizes="(max-width: 700px) 46vw, 300px" onError={() => setFailed(true)} />
...
function FeaturedGallery({ featured, walkthroughs, featuredNowBusinessId, now }: { featured: WeeklyFeaturedProgram; walkthroughs: FeaturedBoothWalkthrough[]; featuredNowBusinessId: number | null; now: number | null }) {
  ...
  if (!featured.booths.length) return <div className="discovery-empty"><h3>Today&apos;s featured lineup is being prepared.</h3><p>More participants will appear as their AfricMade pages are published.</p></div>;
  return <div className="featured-gallery-wrap"><div className="featured-gallery" aria-label={`${featured.title}, ${featured.boothCount} featured businesses`}>
...
function TodayProgramSchedule({ featured, agenda, sponsorPlacements }: { featured: WeeklyFeaturedProgram; agenda: FeaturedProgramAgendaEntry[]; sponsorPlacements: SponsorPlacement[] }) {
  ...
  const sessionSection = (session: "morning" | "afternoon", label: string) => {
    ...
    return <section className={`featured-agenda-session featured-agenda-${session}`} aria-label={`${label} featured schedule`}><header><span>{label}</span><strong>{featuredProgramTimeLabel(resolved.startMinute, resolved.endMinute)} EAT</strong></header><ol>{entries.map((entry) => entry.kind === "booth"
  ...
  return <details className="featured-agenda"><summary><CalendarClock aria-hidden="true" /><span><small>Today&apos;s schedule</small><strong>{sessionSummary || "No presentations scheduled"}</strong></span><em>{featured.boothCount} {featured.boothCount === 1 ? "participant" : "participants"}</em><ChevronDown className="featured-agenda-chevron" aria-hidden="true" /></summary><div className="featured-agenda-body">{sessionSection("morning", "Morning session")}{intermission?.kind === "intermission" ? <div className={`featured-agenda-intermission${intermission.current ? " current" : ""}`} aria-current={intermission.current ? "time" : undefined}><span>{intermission.label}</span><strong>{intermission.timeLabel} EAT</strong></div> : null}{sessionSection("afternoon", "Evening session")}<p>Five-minute changeovers are included between listed presentations unless a sponsor break is shown.</p></div></details>;
}
...
function FeaturedBroadcastStatus({ featured, selectedDay, now, agenda, sponsorPlacements }: { featured: WeeklyFeaturedProgram; selectedDay: WeeklyFeaturedProgram["schedule"][number] | undefined; now: number | null; agenda: FeaturedProgramAgendaEntry[]; sponsorPlacements: SponsorPlacement[] }) {
  ...
  const content = phase === "live"
    ? activeWalkthrough
      ...
      : activeEntry && (activeEntry.kind === "changeover" || activeEntry.kind === "sponsor_break")
        ? { title: activeSponsor ? `Sponsor spotlight · ${activeSponsor.name}` : `${activeEntry.label} · ${activeEntry.timeLabel} EAT`, detail: activeSponsor ? `${activeEntry.timeLabel} EAT · Paid placement` : "The next presentation begins shortly.", action: "Watch live", href: MIRTPAGE_TIKTOK_URL }
  ...
  return <aside className={`featured-program featured-program-${phase}`} aria-label="Daily Featured livestream status"><span><Radio aria-hidden="true" />{featured.isToday ? "Today’s broadcast" : "Program preview"}</span><strong>{content.title}</strong><small>{content.detail}</small>{content.href ? <a href={content.href} target="_blank" rel="noreferrer">{content.action}<ExternalLink aria-hidden="true" /></a> : null}</aside>;
}
...
export default function FeaturedShowroomsWorkspace({ discovery, sponsoredShowrooms }: { discovery: FeaturedShowroomsView; sponsoredShowrooms: SponsorPlacement[] }) {
  return <section className="discovery discovery-featured-experience" aria-label="Daily Featured"><WeeklyFeatured featured={discovery.featured} sponsorPlacements={sponsoredShowrooms} featuredNowBusinessId={discovery.featuredNowBusinessId} /></section>;
}
