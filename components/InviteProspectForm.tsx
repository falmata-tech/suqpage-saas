...
export default function InviteProspectForm({ requestId, businessName, clientName, email = "" }: { requestId:number; businessName:string; clientName:string; email?:string }) {
  ...
  return <form action={action} className="form-grid">
    ...
    <div className="field"><label htmlFor="invite-handle">AfricMade page handle</label><input id="invite-handle" name="handle" defaultValue={businessName.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")} required maxLength={80}/></div>
    <div className="field"><label htmlFor="invite-design">Initial layout direction</label><select id="invite-design" name="designKey" defaultValue="homevibe"><option value="alhaya">Editorial craft</option><option value="usashopet">Clean and natural</option><option value="novatech">Technical precision</option><option value="homevibe">Warm contemporary</option></select><small>This establishes a private starting point. The approved client brief determines the final page design.</small></div>
