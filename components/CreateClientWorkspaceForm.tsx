...
export default function CreateClientWorkspaceForm() {
  ...
  return <form action={action} className="panel form-grid">
    ...
    <div className="field"><label htmlFor="workspace-handle">AfricMade page handle</label><input id="workspace-handle" name="handle" required maxLength={80} placeholder="business-name"/></div>
    <div className="field"><label htmlFor="workspace-design">Initial layout direction</label><select id="workspace-design" name="designKey" defaultValue="homevibe"><option value="alhaya">Editorial craft</option><option value="usashopet">Clean and natural</option><option value="novatech">Technical precision</option><option value="homevibe">Warm contemporary</option></select><small>This establishes a private starting point. The approved client brief determines the final page design.</small></div>
    ...
    <div className="field full"><small>This creates a private draft workspace, not a request or public page. The client submits detailed instructions after accepting the invitation.</small></div>
    {state.error ? <p className="error field full" role="alert">{state.error}</p> : null}
    {state.invitationUrl ? <div className="notice field full" role="status"><strong>Business workspace created.</strong><p>This 72-hour link is shown once. Copy it now and deliver it securely.</p><input aria-label="Single-use business access invitation" readOnly value={state.invitationUrl} onFocus={(event)=>event.currentTarget.select()}/><small>The draft remains private until an approved page revision is published.</small></div> : null}
