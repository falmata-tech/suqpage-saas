...
export async function createPublicInterest(
  ...
  const existing = await dependencies.repository.findPublicDuplicate(ipHash, input.idempotencyKey);
  if (existing) return { ...existing, duplicate: true };

  const rate = await dependencies.rateLimiter.consume(ipHash);
  if (!rate.allowed) throw new RequestError("Too many requests. Try again later.", 429, rate.retryAfterSeconds);

  try {
    const created = await dependencies.repository.createPublicInterest(input, ipHash);
    return { ...created, duplicate: false };
  } catch (error) {
    const duplicate = await dependencies.repository.findPublicDuplicate(ipHash, input.idempotencyKey);
