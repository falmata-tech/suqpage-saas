...
for (const industry of DISCOVERY_INDUSTRIES) {
  ...
  assert.ok(
    Number((db.prepare("SELECT COUNT(*) total FROM sunday_showcase_selections WHERE industry_key=? AND active=1").get(industry.key) as { total: number }).total) >= 5,
    `${industry.label} has an independently curated Sunday cohort`,
  );
