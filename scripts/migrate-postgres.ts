...
if (!directUrl) throw new Error("MIRTPAGE_POSTGRES_DIRECT_URL is required for PostgreSQL migrations.");
...
try {
  ...
  console.log(result.applied.length ? `Applied PostgreSQL migration ${result.applied.join(", ")}.` : "PostgreSQL migrations are current.");
} finally {
  await pool.end();
}
